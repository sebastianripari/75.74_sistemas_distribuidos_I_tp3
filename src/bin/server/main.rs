use std::{sync::{mpsc::{channel, self}, Arc, RwLock}, thread, env, net::TcpListener, time::Duration};

use cleaner_handler::cleaner_handler;
use client_responser::client_responser;
use reddit_meme_analyzer::commons::{utils::{logger::logger_create, middleware::{middleware_connect, middleware_create_channel, middleware_create_exchange, middleware_send_msg_all_consumers, middleware_consumer_end, middleware_send_msg}, socket::{SocketReader, SocketWriter}}, constants::{host::PORT_DEFAULT, opcodes::{OPCODE_POST_END, OPCODE_COMMENT_END}, queues::QUEUE_INITIAL_STATE}};
