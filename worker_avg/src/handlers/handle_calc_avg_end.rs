...
use crate::{
    utils::{logger::Logger, middleware::middleware_send_msg}, constants::queues::{AVG_TO_FILTER_SCORE, QUEUE_TO_CLIENT},
};

fn publish_score_avg(exchange: &Exchange, score_avg: f32) {
    middleware_send_msg(exchange, &score_avg, AVG_TO_FILTER_SCORE);
    middleware_send_msg(exchange, &score_avg, QUEUE_TO_CLIENT)
}
